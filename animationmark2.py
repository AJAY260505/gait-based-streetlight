...
def create_advanced_street_simulation(width, height, persons_results, ambient, weather, temperature):
    # Create a high-quality simulation canvas
    simulation = np.zeros((height, width, 3), dtype=np.uint8)
    
    # Draw gradient sky based on ambient light
    if ambient == 'day':
        # Day sky gradient (light blue to white)
        for y in range(height//2):
            color_ratio = y / (height//2)
            color = (
                int(220 - 50 * color_ratio),
                int(220 - 50 * color_ratio),
                int(255 - 100 * color_ratio)
            )
            cv2.line(simulation, (0, y), (width, y), color, 1)
        
        # Draw sun
        sun_center = (width - 100, 100)
        cv2.circle(simulation, sun_center, 40, (0, 200, 255), -1)
        cv2.circle(simulation, sun_center, 35, (0, 240, 255), -1)
        
    elif ambient == 'evening':
        # Evening sky gradient (orange to purple)
        for y in range(height//2):
            color_ratio = y / (height//2)
            color = (
                int(255 - 100 * color_ratio),
                int(150 - 100 * color_ratio),
                int(100 + 80 * color_ratio)
            )
            cv2.line(simulation, (0, y), (width, y), color, 1)
        
        # Draw moon
        moon_center = (width - 100, 100)
        cv2.circle(simulation, moon_center, 30, (200, 200, 200), -1)
        cv2.circle(simulation, (moon_center[0]-10, moon_center[1]-5), 25, (50, 50, 80), -1)
        ...
        for y in range(height//2):
            color_ratio = y / (height//2)
            color = (
                int(30 - 20 * color_ratio),
                int(30 - 20 * color_ratio),
                int(60 - 40 * color_ratio)
            )
            cv2.line(simulation, (0, y), (width, y), color, 1)
        ...
        for _ in range(50):
            ...
            cv2.circle(simulation, (x, y), 1, (brightness, brightness, brightness), -1)
            if random.random() > 0.8:
                cv2.circle(simulation, (x, y), 2, (brightness, brightness, brightness), -1)
        
        # Draw moon
        moon_center = (width - 100, 100)
        cv2.circle(simulation, moon_center, 30, (220, 220, 220), -1)
        cv2.circle(simulation, (moon_center[0]-10, moon_center[1]-5), 25, (50, 50, 80), -1)
    ...
    # Add ground texture (subtle noise)
    ground_roi = simulation[height//2:height, 0:width]
    noise = np.random.randint(-10, 10, ground_roi.shape, dtype=np.int16)
    ground_roi = np.clip(ground_roi.astype(np.int16) + noise, 0, 255).astype(np.uint8)
    simulation[height//2:height, 0:width] = ground_roi
    
    # Draw road with perspective
    road_top_width = width // 3
    road_bottom_width = width
    road_points = np.array([
        [width//2 - road_top_width//2, height//2],
        [width//2 + road_top_width//2, height//2],
        [width, height],
        [0, height]
    ], np.int32)
    cv2.fillPoly(simulation, [road_points], (40, 40, 40) if ambient == 'night' else (60, 60, 60))
    
    # Draw road markings with perspective
    marking_color = (200, 200, 200) if ambient != 'night' else (150, 150, 150)
    num_markings = 5
    for i in range(num_markings):
        y_pos = height//2 + (i * (height//2) // num_markings)
        marking_width = 20 + (y_pos - height//2) * 60 // (height//2)
        x_center = width//2
        cv2.rectangle(simulation, 
                     (x_center - marking_width//2, y_pos),
                     (x_center + marking_width//2, y_pos + 10),
                     marking_color, -1)
    
    # Draw buildings in the background
    building_colors = [(80, 80, 100), (100, 80, 80), (80, 100, 80), (100, 100, 80)]
    num_buildings = 6
    for i in range(num_buildings):
        building_width = width // (num_buildings * 2)
        building_height = random.randint(100, 250)
        building_x = i * (width // num_buildings)
        building_color = random.choice(building_colors)
        
        if ambient == 'night' and random.random() > 0.7:
            ...
            # Draw windows
            for wy in range(height//2 - building_height + 10, height//2 - 10, 30):
                for wx in range(building_x + 5, building_x + building_width - 5, 15):
                    if random.random() > 0.4:  # Some windows are lit
                        window_color = (random.randint(200, 255), random.randint(200, 255), random.randint(150, 200))
                        cv2.rectangle(simulation, (wx, wy), (wx+10, wy+20), window_color, -1)
        else:
            cv2.rectangle(simulation, 
                         (building_x, height//2 - building_height),
                         (building_x + building_width, height//2),
    ...
    # Draw streetlight poles with more detail
    num_streetlights = 3
    for i in range(num_streetlights):
        pole_x = (i + 1) * (width // (num_streetlights + 1))
        pole_height = height // 3
        pole_width = 8
        
        # Pole
        cv2.rectangle(simulation, 
                     (pole_x, height//2 - pole_height),
                     (pole_x + pole_width, height//2),
                     (80, 80, 80), -1)
        
        # Pole details
        for j in range(3):
            detail_y = height//2 - pole_height + (j * pole_height // 3)
            cv2.rectangle(simulation, 
                         (pole_x - 2, detail_y),
                         (pole_x + pole_width + 2, detail_y + 3),
                         (60, 60, 60), -1)
    ...
    if persons_results:
        ...
        if best:
            ...
            light_intensity = min(0.3 + sp * 20, 0.8)
    
    # Draw streetlight bulbs and light cones
    for i in range(num_streetlights):
        pole_x = (i + 1) * (width // (num_streetlights + 1)) + 4
        bulb_center = (pole_x, height//2 - height//3)
        
        # Draw bulb
        cv2.circle(simulation, bulb_center, 12, light_color, -1)
        cv2.circle(simulation, bulb_center, 8, (light_color[0] + 40, light_color[1] + 40, light_color[2] + 40), -1)
        
        # Draw light cone if light is on
        if light_radius > 0:
            # Create a mask for the light cone with gradient
            light_mask = np.zeros((height, width, 3), dtype=np.uint8)
            pts = np.array([[pole_x, height//2 - height//3], 
                            [pole_x - light_radius, height//2 + 50], 
                            [pole_x + light_radius, height//2 + 50]], np.int32)
            cv2.fillPoly(light_mask, [pts], light_color)
            
            # Add gradient to light cone
            for y in range(height//2 - height//3, height//2 + 50):
                alpha = 1.0 - (y - (height//2 - height//3)) / (height//2 + 50 - (height//2 - height//3))
                alpha = max(0, alpha)
                light_mask[y, :] = (light_mask[y, :] * alpha).astype(np.uint8)
            
            # Blend the light cone with the simulation
            simulation = cv2.addWeighted(simulation, 1.0, light_mask, light_intensity, 0)
    
    # Draw persons in the simulation with more detail
    person_positions = []
    if persons_results:
        for i, person in enumerate(persons_results):
            # Calculate position based on person index with some movement
            x_pos = width // (len(persons_results) + 1) * (i + 1)
            x_pos += int(10 * np.sin(time.time() * 2 + i))  # Slight movement
            y_pos = height//2 + 20  # On the ground
            
            # Draw person with more detail based on gait
            color = person.get('mode_color', (0, 255, 0))
            gait = person.get('gait_status', 'NORMAL')
            
            # Head
            cv2.circle(simulation, (x_pos, y_pos), 12, color, -1)
            cv2.circle(simulation, (x_pos, y_pos), 10, (color[0]//2, color[1]//2, color[2]//2), -1)
            
            # Body
            cv2.line(simulation, (x_pos, y_pos+12), (x_pos, y_pos+45), color, 3)
            
            # Arms - position based on gait
            if gait == "FAST/RUN":
                cv2.line(simulation, (x_pos, y_pos+20), (x_pos-20, y_pos+10), color, 2)
                cv2.line(simulation, (x_pos, y_pos+20), (x_pos+20, y_pos+10), color, 2)
            elif gait == "SLOW":
                cv2.line(simulation, (x_pos, y_pos+20), (x_pos-10, y_pos+30), color, 2)
                cv2.line(simulation, (x_pos, y_pos+20), (x_pos+10, y_pos+30), color, 2)
            else:
                cv2.line(simulation, (x_pos, y_pos+20), (x_pos-15, y_pos+10), color, 2)
                cv2.line(simulation, (x_pos, y_pos+20), (x_pos+15, y_pos+10), color, 2)
            
            # Legs - position based on gait
            if gait == "FAST/RUN":
                cv2.line(simulation, (x_pos, y_pos+45), (x_pos-15, y_pos+65), color, 2)
                cv2.line(simulation, (x_pos, y_pos+45), (x_pos+15, y_pos+65), color, 2)
            elif gait == "SLOW":
                cv2.line(simulation, (x_pos, y_pos+45), (x_pos-5, y_pos+60), color, 2)
                cv2.line(simulation, (x_pos, y_pos+45), (x_pos+5, y_pos+60), color, 2)
            else:
                cv2.line(simulation, (x_pos, y_pos+45), (x_pos-10, y_pos+60), color, 2)
                cv2.line(simulation, (x_pos, y_pos+45), (x_pos+10, y_pos+60), color, 2)
            
            person_positions.append((x_pos, y_pos))
    
    # Add weather effects with more detail
    if weather == 'rain':
        # Draw rain drops with perspective
        for _ in range(100):
            x = random.randint(0, width)
            y = random.randint(0, height//2)
            length = random.randint(5, 15)
            cv2.line(simulation, (x, y), (x+2, y+length), (200, 200, 255), 1)
            
        # Add puddles on the ground
        for _ in range(5):
            x = random.randint(0, width)
            y = random.randint(height//2 + 10, height - 10)
            cv2.ellipse(simulation, (x, y), (20, 5), 0, 0, 360, (100, 100, 150), -1)
            
    elif weather == 'snow':
        # Draw snow flakes with different sizes
        for _ in range(50):
            x = random.randint(0, width)
            y = random.randint(0, height//2)
            size = random.randint(2, 4)
            cv2.circle(simulation, (x, y), size, (255, 255, 255), -1)
            
    # Add information panel
    panel_height = 100
    cv2.rectangle(simulation, (0, height - panel_height), (width, height), (40, 40, 40), -1)
    cv2.rectangle(simulation, (0, height - panel_height), (width, height - panel_height + 2), (100, 100, 100), -1)
    
    # Add stats to panel
    cv2.putText(simulation, f"Detected: {len(persons_results)} people", (10, height - panel_height + 25), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    cv2.putText(simulation, f"Weather: {weather}", (10, height - panel_height + 50), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    cv2.putText(simulation, f"Time: {ambient}", (10, height - panel_height + 75), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    cv2.putText(simulation, f"Temp: {temperature}C", (width - 150, height - panel_height + 25), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    
    if persons_results:
        cv2.putText(simulation, f"Primary Gait: {persons_results[0]['gait_status']}", (width - 250, height - panel_height + 50), 
...
while True:
    ...
    # ---------- CREATE ADVANCED SIMULATION VIEW ----------
    simulation_width = FRAME_W // 2
    simulation_height = FRAME_H
    simulation_view, person_positions = create_advanced_street_simulation(
        simulation_width, simulation_height, persons_results, ambient, weather, temperature
    )
    ...
    cv2.putText(combined_view, "ADVANCED STREET SIMULATION", (FRAME_W + 50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
