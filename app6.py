...
# Use the small v8 model (pretrained on COCO for 80 classes: person, car, dog, cat, etc.)
...
VARIABILITY_THRESHOLD = 0.015    # Increased threshold - tune as needed
SOME_MIN_SPEED = 0.003           # Only call 'unstable' if moving
...
ANIMAL_CLASSES  = {"dog","cat","horse","sheep","cow","bird","elephant","bear","zebra","giraffe"}
...
while True:
    ...
    detection_results = yolo_model(frame_resized, device="cpu", verbose=False)
    objects_found = set()
    vehicle_found = False
    animal_found = False
    ...
    if results.pose_landmarks:
        ...
        if len(ankle_history) == ankle_history.maxlen:
            ...
            # --- Debug print for live calibration (disable or comment later) ---
    ...
    cv2.imshow('All-in-One Smart Streetlight: Pose+YOLO', frame_resized)
