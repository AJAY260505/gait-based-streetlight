...
while True:
    ...
    if ENABLE_YOLO:
        ...
        for p in persons_results:
            ...
            if box:
                ...
                cv2.putText(frame_resized, f"{label}", (x1, max(y1-30, 10)), cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 3)
                # small track id
                if 'track_id' in p:
                    cv2.putText(frame_resized, f"ID:{p['track_id']}", (x1, y2+30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200,200,200), 2)
    ...
    cv2.putText(frame_resized, f'People: {len(persons_results)}  {summary}', (60,120), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,0,0), 3)
    cv2.putText(frame_resized, f'Env:{ambient}, {weather}  City:{detected_city}', (60,180), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (10,70,70), 3)
    cv2.putText(frame_resized, f'Edge AI Privacy', (60,240), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (10,10,10), 3)

    # FPS
    curr_time = time.time()
    fps = int(1/(curr_time-prev_time)) if prev_time > 0 else 0
    prev_time = curr_time
    cv2.putText(frame_resized, f'FPS: {fps}', (1650,60), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (50,255,255), 2)

    # ---------- SIMULATED LIGHT PANEL (right half) ----------
    # base panel area
    panel_x = FRAME_W // 2
    panel_y = 0
    panel_w = FRAME_W - panel_x
    panel_h = FRAME_H

    # decide panel color and pattern by aggregating highest-priority gait
    # priority: Panic/Evacuation > UNSTABLE > FAST/RUN > SLOW > NORMAL > OFF
    panel_color = (30,30,30)
    panel_alpha = 0.6
    panel_text = "Light: OFF"
    # choose highest severity person (very simple priority)
    ...
    if best:
        mode = best.get('gait_status','No Gait')
        panel_text = f"{best.get('light_mode','Adaptive')}: {best.get('cue','')}"
        color = best.get('mode_color', (100,100,100))
        # convert color to 0..255 BGR tuple already provided
        panel_color = color
        # map walking_speed -> alpha intensity (clamp)
        sp = min(max(best.get('walking_speed',0)*50, 0.1), 1.0)  # scale factor tuneable
        panel_alpha = 0.25 + 0.75 * sp

    # draw panel background with alpha
    overlay = frame_resized.copy()
    cv2.rectangle(overlay, (panel_x, panel_y), (FRAME_W, FRAME_H), panel_color, -1)
    cv2.addWeighted(overlay, panel_alpha, frame_resized, 1 - panel_alpha, 0, frame_resized)

    # draw assist stripes if needed
    if best and (best.get('assist_mode') or best.get('light_mode','').lower().startswith('assist')):
        # draw horizontal stripes projected
        stripe_count = 12
        stripe_h = panel_h // stripe_count
        for i in range(stripe_count):
            y0 = i * stripe_h
            if i % 2 == 0:
                cv2.rectangle(frame_resized, (panel_x, y0), (FRAME_W, y0 + stripe_h//2), (255,255,255), -1)
    # draw panic flashing (red) if panic detected
    if best and best.get('light_mode') == "Panic/Evacuation":
        if int(time.time() * 4) % 2 == 0:
            cv2.rectangle(frame_resized, (panel_x, panel_y), (FRAME_W, FRAME_H), (0,0,255), 5)

    # text on panel
    cv2.putText(frame_resized, panel_text, (panel_x + 20, 80), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255,255,255), 2)
    cv2.putText(frame_resized, f"Sound:{sound}   AssistButton:{accessibility_mode}", (panel_x + 20, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255,255,255), 2)
